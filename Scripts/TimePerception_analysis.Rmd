...
knitr::opts_knit$set(root.dir = "/Users/gloria/Desktop/PercAct-Time-Perception-main")
knitr::opts_knit$set(root.dir = normalizePath("/Users/gloria/Desktop/PercAct-Time-Perception-main"))
...
#df <- do.call(rbind, lapply(files, function(file) read.csv(file, header = TRUE)))
...
# make the "task accuracy" variable which is models_correct/models_done
merged_df <- merged_df %>%
  mutate(task_accuracy = case_when(
    Condition == 2 ~ models_correct / models_done,
    Condition == 1 ~ models_correct / models_done,
    Condition == 3 ~ models_correct / models_done
  ))

#Extra: asses another type of productivity that includes task accuracy (I think adjusted productivity might be better than relative productivity but idk)

# make the "adjusted productivity" - taking accuracy into account
merged_df <- merged_df %>%
  mutate(adjust_productivity = case_when(
    Condition == 2 ~ productivity * task_accuracy,
    Condition == 1 ~ productivity * task_accuracy,
    Condition == 3 ~ productivity * task_accuracy
  ))

# make the "relative productivity" - taking accuracy into account
merged_df <- merged_df %>%
  mutate(relat_productivity = case_when(
    Condition == 2 ~ productivity/task_accuracy,
    Condition == 1 ~ productivity/task_accuracy,
    Condition == 3 ~ productivity/task_accuracy
  ))
...
```{r summarize participant info}

#Participant info: number of female, male, other, mean age, sd age, median age
participant_info <- merged_df %>%
  summarise(male_count = sum(Gender == "male", na.rm = TRUE)/12,
            female_count = sum(Gender == "female", na.rm = TRUE)/12,
            other_count = sum(Gender == "other", na.rm = TRUE)/12,
            mean_age = mean(Age),
            max_age = max(Age),
            min_age = min(Age),
            sd_age = sd(Age),
...
#number of participants in each condition by gender (same as the plot after that just differently displayed)
summary_by_condition <- merged_df %>%
...
# Set column names
colnames(summary_by_condition)[2:4] <- c("female", "male", "other")
...
> The total number of subjects that participated in the study is 30 (15 females, 14 males, 1 other), in the age range from 18 to 26 (M = 21.97, SD = 1.71, Mdn = 22).
...
# mean productivity by gender (overall, in all conditions)
merged_df %>% 
  group_by(Gender) %>% 
  summarise(mean_productivity = mean(productivity),
            mean_adjust_productivity = mean(adjust_productivity),
            mean_relat_productivity = mean(relat_productivity),
            mean_task_accuracy = mean(task_accuracy))
...
merged_df %>% 
ggplot(aes(x = factor(Condition), y = task_accuracy, fill = Gender)) +
  geom_boxplot() +
  theme_minimal() +
  labs(title = "Productivity by Condition and Gender", x = "Condition", y = "Productivity") +
  facet_wrap(merged_df$Gender)
